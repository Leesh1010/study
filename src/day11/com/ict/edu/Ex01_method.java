...
public class Ex01_method {
	...
	//      **반환형 : 반드시 호출한 곳으로 되돌아 가는데 실행 결과를 가지고 갈 때 실행 결과의 자료형을 반환형이라고 함 
	...
	//instance 메서드
	public int getTotal2() { // 이름을 다르게 써야함
		int sum = kor + eng + math; // 3번처럼 변수를 만들지 않아도 되지만 되도록 만들것 !!
		
		//반환형이 void가 아니면 맨 마지막에 return 데이터나 해당 데이터를 가지고 있는 자료형 변수 
		return sum; // int이기 때문에 int형 변수를 써주면 됨 !!!
	}
