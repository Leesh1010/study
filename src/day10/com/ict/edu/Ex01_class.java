...
public class Ex01_class {
	//클래스
	//자바에서의 클래스는 객체(Object)를 정으하는 틀 또는 설계도면과 같은 의미로 사용 됨
	//객체란 클래스를 바탕으로 생성 된 것을 객체라고 한다.
	
	// 클래스가 가지고 있는 것 : 데이터 = 멤버필드 (변수와 상수)
	//					  기능 = 동작 = 멤버 메서드
	//                    생성자 = 클래스가 객체로 만들어질 때 한 번 호출 돼서 데이터를 초기화 할 수 있는 기능을 가졌다.
	//                    ** 클래스 이름 = 저장이름 = 파일이름 = 생성자이름
	
	// 클래스 구조 : 헤더,멤버필드,생성자,멤버메서드
	// 1. 헤더 : [접근 제한자] [클래스 종류에 대한 예약어]class 클래스이름{      }
	//    1) 접근제한자 : 클래스, 멤버필드, 멤버메서드, 생성자 모두 사용 할 수 있다.
	//                 외부에서 접근 할 수 있는 권한의 단계를 말합니다.
	//        가) public : 누구나 다 접근 가능
	//        니) protected : 같은 패키지 또는 상속관계에서만 접근 가능 
	...
	//   4) 클래스이름 : 클래스 이름 = 저장이름 = 파일리음 = 생성자이름
	//               첫글자 대문자 나머지 소문자.
	...
	//              모든 글자가 다 대문자, 단어와 단어 사이에 _ 사용 가능
	
	// 3. 멤버메서드 : 동작, 기능, 실행
	//      - 해당 메서드를 호출(실행)하면 해당 메서드 안에 존재하는 내용이 실행됨
	//      - 호출해서 내용이 실행 된 후에는 호출한 곳으로 되돌아간다.(중요!!!!!!!!!!!!)
	...
	//   2) 메서드이름 : 변수와 같음, 첫글자 소문자, 두 단어 이상일 때 단어 첫글자들은 대문자.
	//                숫자는 중간이나 뒤에 사용, 공백은 사용 불가
	...
	//     new 예약어를 사용하면 무조건 heap 메모리에 사용영역(지정영역)을 만든다.
	
	//5. 클래스흘 객체로 만든 후 멤버필드와 멤버메서드에게 접근하는 방법
