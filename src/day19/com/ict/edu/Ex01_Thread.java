...
// 스레드(일꾼) : 프로세스(실행되고 있는 프로그램) 내에서 실행되는 세부 작업 단위
// 멀티스레드 : 하나의 프로세스에서 여러개의 스레드가 병행적으로 처리 되는 것
// 싱글스레드 : 하나의 프로세스에 하나의 스레드가 일처리 하는 것(day01~day18)

// 자바에서 스레드 생성법
// 자바에서는 start()를 호출하여 run()이가 실행 되면 스레드 실행 됐다고 함
// 1. Thread 크랠스 상속 받기 : start(), run() 메서드가 모두 있다.
...
public class Ex01_Thread {
	...
	public static void main(String[] args) {
		// 지금은 싱글스레드다.(즉, 하나의 스레드가 모든일을 다 처리한다.)
